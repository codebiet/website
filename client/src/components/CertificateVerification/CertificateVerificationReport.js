...
const CertificateVerificationReport = React.forwardRef(
  ({ certificateData }, ref) => {
    const clipBodyHeight = () => {
      document.getElementById("app").style.maxHeight =
        window.innerHeight > 600 ? window.innerHeight + "px" : "600px";
    };
    const unclipBodyHeight = () => {
      document.getElementById("app").style.maxHeight = "unset";
    };
    useLayoutEffect(() => {
      window.addEventListener("resize", clipBodyHeight);
      return () => {
        window.removeEventListener("resize", clipBodyHeight);
        unclipBodyHeight();
      };
    }, []);
    useEffect(() => {
