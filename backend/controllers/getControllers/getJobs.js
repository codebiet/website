...
module.exports = async (req, res) => {
  ...
  try {
    ...
    let totalItems = await Job.countDocuments({...queryObj,applyBy: {[statQuery]: date}})
    let jobs = await Job.find({...queryObj,applyBy: {[statQuery]: date}}).limit(limit).skip(skip).sort({_id:-1})
